...
class _SearchWidgetState<T> extends State<SearchWidget<T>> {
	...
	void didChangeDependencies() {
		...
		if (controller == null) {    
			...
			if (widget.initialSearchText?.isNotEmpty ?? false) {
				...
				controller!.search(widget.search(context, textEditingController.text));
			}
		}
	...
	Widget buildAwaitSearch(BuildContext context) {
		...
		return AwaitWidget(
			message: UsefulWidgetsLocalizations.of(context)[SearchWidgetMessages.searchingMessage]
		);

	}

	void search(BuildContext context, String query) {
		controller!.search(widget.search(context, query));
	}
